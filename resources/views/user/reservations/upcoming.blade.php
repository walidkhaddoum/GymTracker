                        ...
                        <div id="content_layout">
                            ...
                            <div class="space-y-8">
                                <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                    ...
                                    <tbody>
                                    @foreach($upcoming_individual_sessions as $individualSession)
                                        <tr>
                                            <td>{{ $individualSession->sessionAssignments->first()->trainer->first_name }} {{ $individualSession->sessionAssignments->first()->trainer->last_name }}</td>
                                            <td>{{ $individualSession->date }}</td>
                                            <td>{{ $individualSession->start_time }}
                                                - {{ $individualSession->end_time }}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                                ...
                                <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                    ...
                                    <tbody>
                                    @foreach($upcoming_group_sessions as $groupSession)
                                        <tr>
                                            <td>{{ $groupSession->name }}</td>
                                            <td>{{ $groupSession->date }}</td>
                                            <td>{{ $groupSession->start_time }} - {{ $groupSession->end_time }}</td>
                                            <td>{{ $groupSession->sessionAssignments->first()->trainer->first_name }} {{ $groupSession->sessionAssignments->first()->trainer->last_name }}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
